...
const BUILD_ID = "v24-latency-aware-ai-20260705";
...
- Treat BUY-ready canonical state as a serious opportunity, not a suggestion to restart the entire checklist. You may still WAIT, but name the specific present-tense failure that outweighs the executable setup.
...
export default function App(){
  ...
  const aiFreezeR=useRef(false),lastMeaningfulAiKeyR=useRef(""),lastActiveWallR=useRef(Date.now());
  ...
  const doTick=useCallback(eng=>{
    ...
    if(shouldAskAI&&!thinkR.current){
      ...
      const applyDecision=(dec,source="AI")=>{
          ...
          addJournal(liveTs,`AI_DECISION_ACCEPTED request:${requestCtx.id} decision:${dec.decision} age:${semantic.ageTicks??(tickR.current-requestCtx.tick)}t/${Math.round(semantic.ageMs??0)}ms mode:${semantic.mode} current:${tradeIntentR.current?.action||"WAIT"}.`);
