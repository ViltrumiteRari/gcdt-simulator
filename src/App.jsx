...
    reasoning:`AI response failed; no new position is permitted without a valid independent AI judgment. Canonical ${intent.direction||"NONE"} remains only an eligibility proposal.`,
...
- Treat canonical state as an eligibility proposal, not an order. You retain final veto authority when causality is weak, data is stale, transmission failed, an active forecast is unresolved, the episode is not genuinely new, or drawdown review is active.
- Separate rule completeness, execution readiness, model confidence, forecast probability, and contract quality. Never describe checklist completion as certainty.
- Non-resolution is evidence: an imminent forecast that misses its window must lose confidence and cannot be silently renewed with the same thesis.
- GEX tension is not directional evidence by itself. Distinguish tension, direction, and actual price transmission.
- A persistent SPX/ITS lead without SPY response becomes TRANSMISSION_FAILED and lowers confidence rather than accumulating stored pressure.
...
  },[done,sessionLabel,sessionMode,selectedReplayDate]);
  ...
  const drainCognition=useCallback(()=>{
    ...
    const lines=batch.map(x=>`${x.t} DATA ${x.dataState||"UNKNOWN"} TRANSMISSION ${x.transmissionState||"UNKNOWN"} | SPY ${x.spy.toFixed(2)} SPX ${x.spx.toFixed(0)} | SPX ITS ${x.itsSPX.toFixed(2)} base ${x.spxItsBase.toFixed(2)} local ${x.spxItsLocal>=0?"+":""}${x.spxItsLocal.toFixed(2)} | SPY ITS ${x.itsSPY.toFixed(2)} base ${x.spyItsBase.toFixed(2)} local ${x.spyItsLocal>=0?"+":""}${x.spyItsLocal.toFixed(2)} | ITS gap ${x.gap>=0?"+":""}${x.gap.toFixed(2)} | FEP distance SPX ${x.spxFepGap>=0?"+":""}${x.spxFepGap.toFixed(1)} / SPY ${x.fepGap>=0?"+":""}${x.fepGap.toFixed(2)} | FEP disagreement ${x.fepDisagreement>=0?"+":""}${x.fepDisagreement.toFixed(2)} | ${x.rubberInterpretation} | GEX ${fmt.gex(x.spxGex)} walls ${x.putWall.toFixed(1)}/${x.callWall.toFixed(1)} intent ${x.intent} ${x.readiness}% local ${x.local} structural ${x.structural}${x.position?` position ${x.position}`:""}`).join("\n");
...
    const rawIntent=buildTradeIntent(m,candR.current,nextBrain,nt,det,chain,posR.current,nc,tradeMemoryR.current);
    ...
    const intent=applyMetacognitiveGates(rawIntent,{dataHealth,transmission,activeForecast:activeForecastNow,drawdownActive:metacognitionR.current.drawdownReview?.active,signalTrust:metacognitionR.current.signalTrust,signalKeys});
...
    setThinking(true);
    try{
      const closingPrompt=`SESSION_CLOSING_REFLECTION\nThe market session is over. No further trade action is possible. You are in a historical simulation, but evaluate every decision with live-capital discipline. Identify the strongest failed belief, evidence overweighted, evidence ignored, every timed-out forecast, whether confidence decayed quickly enough, repeated causal episodes, whether canonical eligibility improperly pressured action, whether exits protected capital, one session-specific lesson, one tentative general lesson, and what must NOT be learned from one session. Review entries, exits, contract decay, signal-to-price transmission and uncertainty without overgeneralizing. Return JSON with private_reflection and next_session_handoff.\nFORECASTS:${JSON.stringify(metacognitionR.current.forecasts)}\nTRADES:${JSON.stringify(tradeDiagnostics)}\nJOURNAL:${JSON.stringify(journalR.current.slice(-80))}`;
