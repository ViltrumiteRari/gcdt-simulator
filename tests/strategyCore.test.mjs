...
assert.match(app, /sessionId:thoughtSessionIdR\.current/);
assert.match(app, /\/session\/start/);
assert.match(app, /\/session\/end/);
assert.match(app, /resetPostExitState=useCallback/);
assert.match(app, /source:"POST_EXIT_RESET"/);
assert.match(app, /resetPostExitState\(why,m\)/);
assert.match(app, /resetPostExitState\("AI_SELL",currentMarket\)/);
assert.match(app, /resetPostExitState\("DEFAULT_0DTE_CUTOFF_15_45",m\)/);
assert.match(app, /controller\?\.abort\(`POSITION_CLOSED_\$\{reason\}`\)/);
const agentMain = fs.readFileSync(new URL('../electron/main.cjs', import.meta.url), 'utf8');
...
assert.match(meetingOrchestrator, /memos\.jsonl/);
assert.match(meetingOrchestrator, /shared-notepad\.txt/);
