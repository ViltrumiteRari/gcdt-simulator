...
function createReplayEngine(replayData){
  ...
  function tick(){
    ...
    let{h,m}=s;m++;if(m>=60){m=0;h++;}
    const newPre=h<OPEN_H||(h===OPEN_H&&m<OPEN_M);
    s={...s,spySpot:newSpySpot,spxSpot:newSpxSpot,fep:newFep,accelerator:newAccel,netGex:netGexSpy,netGexSpx:spx.gex,callWall:newCallWall,itsSPX:newItsSPX,itsSPY:newItsSPY,callDom:spx.callDom,callDomSpyEst:newCallDomSpyEst,ndf:newNdf,dealerPct:newDealer,iv:newIv,pcr:newPcr,gexInfluence:gi,tick:t+1,h,m,isPremarket:newPre,isTradeable:!newPre,spxCallDomBuffer:spxBuf,synthData:spx.synth};
...
export default function App(){
  ...
  const startSession=useCallback((mode)=>{
    ...
    prevAccelR.current=0;lastAiTickR.current=-99;repeatWaitR.current=0;lastWaitReasonR.current="";lastMindsetKeyR.current="";optionMemoryR.current={};sessionModelR.current={leadOpp:0,leadCatch:0,leadReject:0,accelFollow:0,accelFail:0,pinWins:0,pinLosses:0,lastLeadState:"",lastAccelTick:-99};
